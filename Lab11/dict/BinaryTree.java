...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
    ...
    if (node == null) {
        ...
        // key smaller than node key
        return findHelper(key, node.leftChild);

    } else {
        // key larger than node key
  ...
  public Entry remove(Object key) {
    ...
    if (node == null) {
        ...
        if (node.leftChild == null && node.rightChild == null) {
            ...
            // node has one child, move node's child to take its place
            ...
            // find min x in the right subtree
            BinaryTreeNode minNode = findMin(node.rightChild);
            // remove x (x has no left child)
            if (((Comparable) minNode.entry.key()).compareTo(minNode.parent.entry.key()) < 0) {
                // minNode is the left child
                minNode.parent.leftChild = minNode.rightChild;
            } else {
                // minNode is the right child
                minNode.parent.rightChild = minNode.rightChild;
            }
            // replace node's entry with x's entry
  ...
  public BinaryTreeNode findMin(BinaryTreeNode node) {
    BinaryTreeNode currentMin = node;
    while (node != null) {
        if (((Comparable) node.entry.key()).compareTo(currentMin.entry.key()) < 0) {
            currentMin = node;
        }
        node = node.leftChild;
    }
  ...
  public static void main(String[] args) {
    ...
    tree.insert(Integer.valueOf(1), "A");
    tree.insert(Integer.valueOf(6), "V");
    tree.insert(Integer.valueOf(3), "K");
    tree.insert(Integer.valueOf(2), "Z");
    tree.insert(Integer.valueOf(5), "L");
    tree.insert(Integer.valueOf(9), "L");
    ...
    tree.insert(Integer.valueOf(7), "S");
    tree.insert(Integer.valueOf(8), "X");
    tree.insert(Integer.valueOf(10), "B");
