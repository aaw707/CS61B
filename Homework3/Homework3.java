...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    int index = 1;
    // use counter to prevent infinite looping
    for (int counter = 1; counter < ints.length; counter++) {
      // if current element equals to the previous one
      if (ints[index] == first) {
        // switch the remaining element one position to the left
        for (int j = index; j < ints.length - 1; j++) {
          ints[j] = ints[j + 1];
        }
        // set the current element to be -1 at the end of the array
        ints[ints.length - 1] = -1;
      } else {
        // set the current element for the next comparision
        first = ints[index];
