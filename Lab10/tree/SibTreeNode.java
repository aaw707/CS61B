...
class SibTreeNode extends TreeNode {
  ...
  public TreeNode parent() throws InvalidNodeException {
    ...
    // if this is not a valid node, throw an exception
    if (!isValidNode()) {
        throw new InvalidNodeException();
    }
    // if the node is root, return an invalid node
    if (parent == null) {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if(isValidNode()) {
        System.out.println("this:" + this.item);
        ...
        if (firstChild == null) {
            ...
            // go though children to find cth position
            while ((kid != null) && (c > 1)) {
                prev = kid;
                kid = kid.nextSibling;
                c--;
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (isValidNode()) {
        System.out.println("to be removed:" + this.item);
        // if not root with no child
        if (parent != null) {
            if (firstChild != null) {
                // if not a leaf, do nothing
                return;
            } else {
                // if is a leaf
                ...
                if (kid == this) {
                    ...
                    // if it has left siblings
                    SibTreeNode prev = null;
                    while (kid != this) {
                        // iterate from the first sibling to the prev
                        prev = kid;
                        kid = kid.nextSibling; 
                    }    
